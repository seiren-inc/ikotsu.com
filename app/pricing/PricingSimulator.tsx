...
import { HIGHWAY_ICS } from '@/lib/data/highway-tolls';
...
export default function PricingSimulator({ sizeOptions, conditionOptions }: Props) {
  ...
  const [selectedIC, setSelectedIC] = useState<string>(HIGHWAY_ICS[0].id);
  ...
  const filteredICs = HIGHWAY_ICS.filter(ic => 
  ...
  // 出張費・高速代金の計算
  const icData = HIGHWAY_ICS.find(ic => ic.id === selectedIC) || HIGHWAY_ICS[0];
